...
def get_init_latent(orig_pic,gan_gen):
    latent_code_init_not_trunc = torch.randn(1, 512).cuda()
    with torch.no_grad():
         _, latent_code_init = gan_gen([latent_code_init_not_trunc], return_latents=True,
                                       truncation=0.7, truncation_latent=gan_gen.mean_latent(4096)
)
...
def optim(text, input_img, opts, region):
    ...
    latent_code_init  = get_init_latent(orig_pic, gan_generator)



    # 生成初始图片
    with torch.no_grad():
        inv_img, _ = gan_generator([latent_code_init], input_is_latent=True, randomize_noise=False)
    ...
    for i in pbar:
        ...
        img_gen, _ = gan_generator([latent], input_is_latent=True, randomize_noise=False)
...
if __name__ == '__main__':
    ...
    result = optim(text='red hair', input_img='input_img/img1.png', opts=opts, region=None)
